...
This'll create a `sst.config.ts` file in your project root and install SST.

---

## 2. Add a Cluster

To deploy our Bun app, let's add a [AWS Fargate](https://aws.amazon.com/fargate/) container with [Amazon ECS](https://aws.amazon.com/ecs/). Update your `sst.config.ts`.
...
This creates a VPC with a bastion host, a ECS Cluster, and adds a Fargate service to it.
