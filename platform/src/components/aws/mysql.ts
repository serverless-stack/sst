...
export interface MysqlArgs {
  /**
   * The Mysql engine version. Check out the [available versions in your region](https://docs.aws.amazon.com/AmazonRDS/latest/AuroraUserGuide/Concepts.Aurora_Fea_Regions_DB-eng.Feature.ServerlessV2.html#Concepts.Aurora_Fea_Regions_DB-eng.Feature.ServerlessV2.amy).
   * 
   * See the `aws rds describe-db-engine-versions` to list engine versions.
   * 
   ...
   *
   * :::warning
   * Aurora Serverless v2 supports scale to 0 for MySQL 3.08.0+.
   * However, the minimum cost of a MySQL cluster with 0.5 ACU per month is roughly `0.5 * $0.12 per ACU hour * 24 hrs * 30 days = $43.20`.
   *
  ...
  scaling?: Input<{
     ...
     * - The max connections for a 0.5 ACU Mysql instance is capped at 2000.
     *
     * @default `0 ACU`
     * @example
     * ```js
    ...
    max?: Input<ACU>;
    /**
     * The time in seconds before the Aurora DB cluster is paused. Values values are 300 (5 minutes) through 86400 (24 hours).
     *
     * @default 300
     * @example
     * ```js
     ...
     * ```
     */
    secondsUntilAutoPause?: Input<number>;
  }>;
  /**
 ...
 *
 * This version allows the database, which uses AWS RDS Aurora Serverless v2, to use the RDS
 * Data API. This allows your machine to connect to the database during "sst dev" without
 * the need for a VPN.
 *
 * :::note
 * Data API for Aurora Mysql Serverless v2 is still being [rolled out in all regions](https://docs.aws.amazon.com/AmazonRDS/latest/AuroraUserGuide/Concepts.Aurora_Fea_Regions_DB-eng.Feature.ServerlessV2.html#Concepts.Aurora_Fea_Regions_DB-eng.Feature.ServerlessV2.apg).
 * :::
 *
...
export class Mysql extends Component implements Link.Linkable {
  ...
  constructor(
    ...
    function normalizeScaling() {
      return output(args.scaling).apply((scaling) => ({
        ...
        secondsUntilAutoPause: scaling?.secondsUntilAutoPause ?? 300,
      }));
    }

    function normalizeVersion() {
      return output(args.version).apply((version) => version ?? "3.08.0");
    }
    ...
    function createCluster() {
      return new rds.Cluster(
        ...transform(
          ...
          `${name}Cluster`,
          {
            engine: rds.EngineType.AuroraMysql,
  ...
  /**
   * Reference an existing Mysql cluster with the given cluster name. This is useful when you
   * create a Mysql cluster in one stage and want to share it in another. It avoids having to
   * create a new Mysql cluster in the other stage.
   *
   * :::tip
   * You can use the `static get` method to share Mysql clusters across stages.
   * :::
   *
   * @param name The name of the component.
   * @param clusterID The id of the existing Mysql cluster.
   *
   * @example
   ...
   *   ? sst.aws.Mysql.get("MyDatabase", "app-dev-mydatabase")
   *   : new sst.aws.Mysql("MyDatabase");
   * ```
   *
